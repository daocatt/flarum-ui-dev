...
import PostLoading from 'flarum/forum/components/LoadingPost';
import ReplyPlaceholder from 'flarum/forum/components/ReplyPlaceholder';
import Button from 'flarum/common/components/Button';
import ItemList from 'flarum/common/utils/ItemList';

/**
 * The `PostFirstStream` component displays an infinitely-scrollable wall of posts in
 ...
 * - `onPositionChange`
 */
export default class PostFirstStream extends Component {
  oninit(vnode) {
    super.oninit(vnode);
  ...
  view() {
    ...
    const viewingEnd = false;
    const posts = this.stream.posts();
    const postFirst = posts.shift();
    ...
    const itemsFunc = (post) => {
      let content;
      const i = 0;
      const attrs = { 'data-index': visibleStart + i };
    ...
    const items = itemsFunc(postFirst);
  ...
  loadPostsIfNeeded(top = window.pageYOffset) {
    ...
    if (visibleStart > 0) {
      const $item = this.$('.PostStream-item[data-index=' + visibleStart + ']');
    ...
    if (visibleEnd < this.stream.count()) {
      const $item = this.$('.PostStream-item[data-index=' + (visibleEnd - 1) + ']');
